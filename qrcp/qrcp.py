...
import http.server, html, socket, socketserver
import sys, os, random, pathlib, platform, re
from shutil import make_archive, copyfileobj
import signal, argparse, qrcode, base64
...
def clean_exit():
  ...
  print("\r", end="")  # Returning the cursor to home
  print("  ")  # printing space over it to hide ^C
  sys.exit()
...
def FileUploadServerHandlerClass(output_dir, auth, debug):
  ...
  class FileUploadServerHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_AUTHHEAD(self):
      self.send_response(401)
      self.send_header('WWW-Authenticate', 'Basic realm=\"qrcp\"')
      self.send_header('Content-type', 'text/html')
      self.end_headers()
    ...
    def do_POST(self):
      ...
      f.write(
          ("<span style=\"font-family: 'Comfortaa', cursive;color:'#263238';\">%s</span><br>" % info).encode())
      f.write(("<br><a href=\"%s\" style=\"font-family: 'Comfortaa', cursive;color:'#263238';\">back</a>" %
    ...
    def send_head(self):
      ...
      f.write(b"<center>")
      f.write(
          b"<img src=\"https://raw.githubusercontent.com/sdushantha/qr-filetransfer/master/logo.png\">")
...
def get_ssid():
  ...
  if operating_system == "Darwin":
    ...
    if not ssid:
      ssid = os.popen(
          "nmcli -t -f active,ssid dev wifi | egrep '^yes' | cut -d\\' -f2 | sed 's/yes://g' 2>/dev/null").read().strip()
    return ssid
...
def get_local_ips_available():
  ...
  try:
    ...
    for iface in netifaces.interfaces():
      ips.extend([x["addr"] for x in netifaces.ifaddresses(
          iface).get(netifaces.AF_INET, []) if x and "addr" in x])
...
def start_upload_server(file_path, debug, custom_port, ip_addr, auth):
  ...
  try:
    ...
    print("This port seems to be in use. Please try another port!")
    clean_exit()
...
def main():
  ...
  parser.add_argument('--ip_addr', dest="ip_addr",
                      choices=get_local_ips_available(), help="specify IP address")
  parser.add_argument('--auth', action="store",
                      help="add authentication, format: username:password", type=b64_auth)
  parser.add_argument(
      "--no-force-download",
      action="store_true",
  ...
  if args.receive:
    start_upload_server(file_path=args.file_path, debug=args.debug,
                        custom_port=args.port, ip_addr=args.ip_addr, auth=args.auth)
  else:
